...
export function SettingsModal() {
  ...
  const timezoneOptions = useMemo(() => {
    ...
    const activeManual = ensureValidTimezone(manualTimezone ?? FALLBACK_TIMEZONE);
    const withManual = [activeManual, ...zones];

    return Array.from(new Set(withManual)).sort((a, b) => a.localeCompare(b));
  }, [manualTimezone]);
  ...
  const activeTimezone = ensureValidTimezone(
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 p-6 backdrop-blur-xl"
      role="dialog"
      aria-modal="true"
