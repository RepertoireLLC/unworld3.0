        ...
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              aria-expanded={!collapsedSections.anomalies}
            ...
            >
              <ChevronDown
                className={`h-4 w-4 transition-transform ${collapsedSections.anomalies ? '-rotate-90' : ''}`}
              />
              <span className="sr-only">{collapsedSections.anomalies ? 'Expand analytic dashboard' : 'Collapse analytic dashboard'}</span>
            </button>
          </div>
        </div>
      ...
      <section className="rounded-[32px] border border-white/10 bg-white/5 p-6 shadow-[0_20px_70px_-40px_rgba(236,72,153,0.5)]">
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <ChevronDown
                className={`h-4 w-4 transition-transform ${collapsedSections.notes ? '-rotate-90' : ''}`}
              />
              <span className="sr-only">{collapsedSections.notes ? 'Expand tactical annotations' : 'Collapse tactical annotations'}</span>
            </button>
          </div>
        </div>
                ...
                {savedLogs.length > 0 ? (
                  savedLogs.map((log) => (
                    ...
                    >
                      ...
                      <div className="mt-3 flex items-center justify-end gap-2 text-xs text-white/50">
                        <button
                          onClick={() =>
                            setSavedLogs((prev) => prev.filter((entry) => entry.id !== log.id))
                          }
                          className="rounded-full border border-white/10 bg-white/10 px-3 py-1 uppercase tracking-[0.3em] transition hover:border-white/30 hover:bg-white/20"
                        >
                          Remove
                        </button>
                        <button
                          onClick={() =>
                            setSavedLogs((prev) => [
                              { ...log, id: Date.now().toString(), timestamp: Date.now() },
                              ...prev,
                            ])
                          }
                          className="flex items-center gap-2 rounded-full border border-white/10 bg-white/10 px-3 py-1 uppercase tracking-[0.3em] text-white/60 transition hover:border-white/30 hover:bg-white/20"
                        >
