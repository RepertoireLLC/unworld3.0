...
import { BraveBookmark, BraveDownload, BraveTabInit, useBrowserStore } from '../../store/browserStore';
...
const attachmentTypeFromMime = (mime: string): BraveDownload['type'] => {
...
export function BraveWorkspace() {
  ...
  const tabs = useBrowserStore((state) => {
    if (!userId) return [];
    return state.tabsByUser[userId] ?? [];
  });
  const activeTabId = useBrowserStore((state) => {
    if (!userId) return null;
    return state.activeTabByUser[userId] ?? null;
  });
  const bookmarks = useBrowserStore((state) => {
    if (!userId) return [];
    return state.bookmarksByUser[userId] ?? [];
  });
  const downloads = useBrowserStore((state) => {
    if (!userId) return [];
    ...
    async (query: string) => {
      ...
      try {
        const response = await fetch(
          `https://api.duckduckgo.com/?q=${encodeURIComponent(trimmed)}&format=json&no_redirect=1&no_html=1&t=enclypse-brave`
        );
        ...
        console.error('Enclypse Brave discovery failed', error);
  ...
  const handleBookmarkOpen = (bookmark: BraveBookmark) => {
    if (!userId || !activeTab) return;
    ...
    try {
      const tasks = Array.from(fileList).map(async (file) => {
        ...
        logDownload(userId, {
          ...
          iv: encrypted.iv,
          source: activeTab?.url ?? 'enclypse://brave',
        });
      });
      ...
      console.error('Failed to encrypt brave workspace upload', error);
  ...
  if (!userId) {
    return (
      <div className="brave-workspace brave-workspace--empty" role="presentation">
        Sign in to activate the Enclypse Brave workspace.
      </div>
    );
  }

  return (
    <section className="brave-workspace" aria-label="Enclypse Brave encrypted browser">
      <header className="brave-workspace__tabs" role="tablist">
        {tabs.map((tab) => {
          const isActive = tab.id === activeTab?.id;
          return (
            <div key={tab.id} role="presentation" className="brave-workspace__tab-wrapper">
              <button
                type="button"
                role="tab"
                aria-selected={isActive}
                className={`brave-workspace__tab ${isActive ? 'active' : ''}`}
                onClick={() => handleSelectTab(tab.id)}
              >
                <span className="brave-workspace__tab-title">{tab.title || tab.url}</span>
                <span className="sr-only">{isActive ? '(current tab)' : ''}</span>
              </button>
              <button
                type="button"
                className="brave-workspace__tab-close"
        ...
        <button type="button" className="brave-workspace__tab brave-workspace__tab--new" onClick={handleNewTab}>
          <Plus className="h-4 w-4" aria-hidden="true" />
          <span>New Tab</span>
        </button>
      </header>

      <div className="brave-workspace__toolbar" role="region" aria-label="Browser toolbar">
        <div className="brave-workspace__toolbar-main">
          <button type="button" className="brave-workspace__icon-button" onClick={handleReload} aria-label="Reload tab">
            <RefreshCw className="h-4 w-4" />
          </button>
          <form className="brave-workspace__url-form" onSubmit={handleNavigate} aria-label="Navigate to URL">
            <input
              className="brave-workspace__url-input"
              value={urlInput}
        ...
        <div className="brave-workspace__toolbar-actions">
          <button
            type="button"
            className={`brave-workspace__chip ${activeTab?.shieldsUp ? 'active' : ''}`}
          ...
          <button
            type="button"
            className={`brave-workspace__chip ${activeTab?.isPrivate ? 'active' : ''}`}
          ...
          <button type="button" className="brave-workspace__chip" onClick={handleAddBookmark}>
      ...
      <div className="brave-workspace__status" role="region" aria-live="polite">
        <div className="brave-workspace__signal">
        ...
        <div className="brave-workspace__keywords" role="list" aria-label="Focus terms">
          {(activeTab?.trustedKeywords ?? []).map((keyword) => (
            <span key={keyword} className="brave-workspace__keyword" role="listitem">
              #{keyword}
            </span>
          ))}
        </div>
        <div className="brave-workspace__upload">
          <label className="brave-workspace__chip" htmlFor="brave-upload-input">
            <Upload className="h-4 w-4" aria-hidden="true" />
            Beam Attachments
          </label>
          <input
            id="brave-upload-input"
            type="file"
            ref={fileInputRef}
            className="sr-only"
            multiple
          ...
          />
          {uploadStatus && <p className="brave-workspace__status-note">{uploadStatus}</p>}
        </div>
      </div>

      <div className="brave-workspace__discovery" role="region" aria-label="Organic discovery">
        <form className="brave-workspace__search" onSubmit={handleDiscovery}>
        ...
        {searchError && <div className="brave-workspace__alert" role="status">{searchError}</div>}
        {filteredOutCount > 0 && (
          <div className="brave-workspace__alert brave-workspace__alert--quiet" role="status">
            Filtered out {filteredOutCount} suspected promotions.
          </div>
        )}
        <div className="brave-workspace__results" role="list">
          {isSearching ? (
            <div className="brave-workspace__result brave-workspace__result--loading">
              <Loader2 className="h-4 w-4 animate-spin" aria-hidden="true" />
              <span>Decrypting noise from the wider web…</span>
            </div>
          ) : (
            searchResults.map((result) => (
              <article key={result.url} className="brave-workspace__result" role="listitem">
                <header>
                  <h3>{result.title}</h3>
                ...
                <p>{result.snippet}</p>
                <footer>
                  <span className="brave-workspace__result-meta">Relevance {result.relevance.toFixed(1)}</span>
                  <span className="brave-workspace__result-meta">
          ...
          {!isSearching && searchResults.length === 0 && !searchError && (
            <div className="brave-workspace__result brave-workspace__result--empty">
      ...
      <div className="brave-workspace__content">
        <div className="brave-workspace__frame" aria-live="polite">
          {isLoadingFrame && (
            <div className="brave-workspace__frame-loading">
          ...
          {activeTab && (
            <iframe
              ...
              className="brave-workspace__iframe"
              sandbox="allow-same-origin allow-scripts allow-forms allow-popups"
            />
          )}
        </div>
        <aside className="brave-workspace__intel" aria-label="Saved sources and encrypted transfers">
          <section className="brave-workspace__panel" aria-label="Trusted sources">
            <header>
              <Bookmark className="h-4 w-4" aria-hidden="true" />
              <h4>Trusted Sources</h4>
            </header>
            {bookmarks.length === 0 ? (
              <p className="brave-workspace__empty">Save a page to pin it here.</p>
            ) : (
              <ul>
                {bookmarks.map((bookmark) => (
                  <li key={bookmark.id}>
                    ...
                    <div className="brave-workspace__panel-actions">
                      <span>{bookmark.url}</span>
                      <button
                        type="button"
                        className="brave-workspace__icon-button"
          ...
          <section className="brave-workspace__panel" aria-label="Encrypted downloads">
            <header>
              <Download className="h-4 w-4" aria-hidden="true" />
              <h4>Encrypted Transfers</h4>
            </header>
            {downloads.length === 0 ? (
              <p className="brave-workspace__empty">Beam a file to populate your secure download log.</p>
            ) : (
              <ul>
                {downloads.map((download) => (
                  <li key={download.id}>
                    <div>
                      <p>{download.name}</p>
                      <span className="brave-workspace__download-meta">
                        {attachmentTypeFromMime(download.mimeType)} · {formatBytes(download.size)} · {formatTimestamp(download.timestamp)}
                      </span>
                    </div>
                    <span className="brave-workspace__download-source">{download.source}</span>
                  </li>
                ))}
              </ul>
            )}
