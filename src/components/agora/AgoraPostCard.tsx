  ...
  return (
    <article
      ...
      className={classNames(
        'relative overflow-hidden rounded-2xl border border-white/10 bg-slate-950/70 p-6 transition-shadow duration-300',
        entry.curiosityBoosted ? 'shadow-lg shadow-fuchsia-500/10' : 'shadow-md shadow-black/30'
      )}
    >
      ...
      <header className="relative z-10 flex flex-wrap items-center gap-3 text-xs font-medium uppercase tracking-[0.2em] text-white/60">
        <span className={classNames('rounded-full border px-3 py-1 text-white', labelStyle)}>
          {entry.label}
        </span>
      ...
      <div className="relative z-10 mt-4 space-y-4">
        <div>
          <h3 className="text-xl font-semibold text-white">{entry.post.title}</h3>
        ...
        <div className="flex flex-wrap items-center gap-2">
          {entry.post.tags.map((tag) => {
            const followed = isTagFollowed?.(tag) ?? false;
            return (
              <div
                key={tag}
                className={classNames(
                  'flex items-center gap-2 rounded-full border px-3 py-1 text-xs uppercase tracking-[0.2em]',
                  followed
              ...
              >
                <button
                  ...
                  className="flex items-center gap-2 text-current transition hover:opacity-80"
                >
                ...
                {onToggleFollowTag && (
                  <button
                    ...
                    className="rounded-full border border-white/10 bg-white/10 p-1 text-white/60 transition hover:bg-white/20"
                    aria-label={followed ? 'Unfollow tag' : 'Follow tag'}
                  >
                    {followed ? (
                      <BookmarkCheck className="h-3.5 w-3.5 text-emerald-300" />
                    ) : (
                      <BookmarkPlus className="h-3.5 w-3.5 text-white/60" />
                    )}
                  </button>
                )}
              </div>
        ...
        <div className="flex flex-wrap items-center gap-4 text-sm text-white/60">
          <span className="flex items-center gap-2">
            <Sparkles className="h-4 w-4 text-emerald-300" /> Resonance {(entry.similarity * 100).toFixed(0)}%
          </span>
          <span className="flex items-center gap-2">
            <Layers className="h-4 w-4 text-sky-300" /> Engagement {entry.post.engagement_score.toFixed(1)}
          </span>
        </div>

        {transparencyEnabled && matches.length > 0 && (
          <div className="rounded-xl border border-cyan-400/30 bg-cyan-500/10 p-4 text-sm text-white/70">
            <p className="flex items-center gap-2 text-xs font-medium uppercase tracking-[0.2em] text-cyan-200">
              ...
              {matches.map((match) => (
                <li key={match.topic} className="flex items-center justify-between">
                  ...
                  <span className="flex items-center gap-2 text-white/60">
                    <span className="rounded-full bg-white/10 px-2 py-0.5 text-[11px] uppercase tracking-[0.2em]">
        ...
        <div className="flex flex-wrap items-center gap-3 text-sm text-white/60">
          <button
            type="button"
            onClick={handleLike}
            className="flex items-center gap-2 rounded-lg border border-emerald-400/40 bg-emerald-500/10 px-4 py-2 text-white transition hover:bg-emerald-500/20"
          >
          ...
          <button
            ...
            className="flex items-center gap-2 rounded-lg border border-sky-400/40 bg-sky-500/10 px-4 py-2 text-white transition hover:bg-sky-500/20"
          >
            <MessageCircle className="h-4 w-4" />
            Thread {comments.length}
          </button>
          {entry.post.mediaType === 'video' && (
            <span className="flex items-center gap-2 rounded-lg border border-fuchsia-400/40 bg-fuchsia-500/10 px-4 py-2 text-white">
          ...
          <div className="space-y-4">
            <div className="rounded-xl border border-white/10 bg-white/10 p-4">
              <textarea
                ...
                className="h-24 w-full rounded-lg border border-white/10 bg-slate-950/60 px-4 py-3 text-sm text-white placeholder:text-white/40 focus:border-emerald-400/40 focus:outline-none"
              />
              <div className="mt-3 flex justify-end">
                <button
                  type="button"
                  onClick={handleSubmitComment}
                  className="rounded-lg border border-emerald-400/40 bg-emerald-500/10 px-4 py-2 text-sm text-white transition hover:bg-emerald-500/20"
                >
                  Post comment
                </button>
              </div>
                ...
                comments.map((comment: ForumComment) => (
                  ...
                  >
                    <div className="flex items-center justify-between text-xs font-medium uppercase tracking-[0.2em] text-white/60">
                      <span>Comment {comment.comment_id.slice(-4)}</span>
