...
export function AINodes() {
  ...
  const openChatSessions = useAIChatStore((state) => state.openChats);
  const openChatForConnection = useAIChatStore((state) => state.openChat);
  ...
  const activeChatIds = useMemo(
    () => new Set(openChatSessions.map((session) => session.connectionId)),
    [openChatSessions]
  );
    ...
    <group ref={groupRef}>
      {nodes.map(({ connection, position }) => (
        <AINode
          ...
          position={position}
          isActive={
            connection.id === activeConnectionId || activeChatIds.has(connection.id)
          }
          onActivate={() => {
            setActiveConnection(connection.id);
            openChatForConnection(connection.id);
          }}
        />
      ))}
    </group>
