...
const scoreRelay = (relay: WifiRelay) => {
  const bandwidthScore = relay.bandwidthMbps ?? 0;
  const latencyPenalty = relay.latencyMs ?? 0;
  const shareBoost = relay.shareable ? 12 : 0;
  const peerBoost = relay.type === 'peer' ? 24 : 0;
  const bluetoothPenalty = relay.viaBluetooth ? 6 : 0;

  return bandwidthScore + shareBoost + peerBoost - latencyPenalty - bluetoothPenalty;
};

const pickOptimalRelayId = (relays: WifiRelay[], previousId: string | null | undefined) => {
  if (!relays.length) {
    return null;
  }

  if (previousId) {
    const existing = relays.find((relay) => relay.id === previousId);
    if (existing) {
      return previousId;
    }
  }
  ...
  return relays
    .slice()
    .sort((a, b) => scoreRelay(b) - scoreRelay(a))[0]?.id ?? null;
};
...
export const useBluetoothStore = create<BluetoothState>((set, get) => ({
  ...
  registerWifiRelays: (peerA, peerB, relays) => {
    ...
    const nextActive = pickOptimalRelayId(nextRelays, base.activeWifiId);
